import type { Post } from '../types'

const post: Post = {
  slug: 'de-schaduwbank-van-private-equity',
  title: 'De (Schaduw)bank van Private Equity',
  author: 'Team Bluemetric',
  date: '2024-09-01',
  tag: 'Private Equity',
  image: '/images/blog/schaduwbank-private-equity.png',
  body: `It is no longer a secret that the historical outperformance of private equity is driven in significant part by debt financing, or leverage. Depending on the time period, leverage accounts for approximately 60 percent (before 2000) to 40 percent (since 2008) of the total returns of the asset class. A less researched phenomenon is which parties are willing to provide this debt.

Nowadays, large private equity transactions are financed to a significant extent by leveraged loans that are broadly syndicated from multiple investors. The loans have a senior position in the capital structure, variable interest rates, and relatively low creditworthiness, and are tradable on the secondary market. In the U.S., the size of the leveraged loan market is $1.4 trillion, of which private equity transactions account for approximately 65%. For comparison, the net intrinsic value of all buyout funds in the U.S. was approximately $1.8 trillion at the end of last year.

The remaining question is who ultimately the investors in the leveraged loans are. The main players are Collateralized Loan Obligations (CLOs), which purchase no less than 70% of the loans. CLOs are actively managed entities, established to assemble broadly diversified portfolios of loans. Subsequently, the CLO is in turn financed by the issuance of equity (approximately 10%) and multiple debt tranches (approximately 90%), which are paid out in order of priority within the capital structure. Thus, a CLO redistributes risk among its financiers according to their risk tolerance.

Investments in CLO equity bear initial credit losses but also offer attractive return potential (approximately 12% per annum since 2003). Nevertheless, CLO equity constitutes a relatively limited portion of illiquid investment portfolios, compared to more prominent investment categories such as private equity.

![](/images/blog/schaduwbank-private-equity.png)

Common reasons for not investing in CLOs are higher complexity and confusion with the infamous Collateralized Debt Obligations (CDOs), known from the 2008 financial crisis. Besides the acronym appearing similar to CLOs, they have no financial exposure correspondence. Furthermore, some investors believe CLOs provide double exposure to companies held by private equity managers. However, while the underlying companies show some overlap, the risk-return profile of CLO equity is actually complementary to private equity and forms a valuable addition to the portfolio. These diversification benefits are driven by several components:

**1. Senior Capital Position** — Leveraged loans have first lien in case of bankruptcy, making actual credit losses relatively limited (the historical recovery rate exceeds 70%). Meanwhile, private equity investors typically lose their investment in bankruptcy.

**2. Contractual Cash Flows** — Coupon payments from leveraged loans provide income under most market conditions. To illustrate: in 2022, when both listed equities and bonds declined (respectively -13.3% and -10.5%), CLO equity delivered an average return of +11.8%.

**3. Active Management** — CLO managers typically may reinvest early-repaid principals in new loans. Thus, during periods of market volatility (e.g., around the financial crisis and the Covid pandemic), one can purchase discounted loans. Private equity experiences headwinds during these years.

**4. Interest Rate Impact** — Because both leveraged loans and CLO debt tranches have variable interest rates, CLO equity returns are relatively robust to rising rates. However, for private equity investors, returns are pressured by higher interest costs experienced by their companies.

On a vintage basis, there is clear diversification benefit visible from including both investment categories to stabilize returns of an illiquid portfolio.

![](/images/blog/schaduwbank-fig2.png)

Additionally, CLOs offer early and stable distributions that can be used to finance other cash flow needs (such as private equity capital calls). All in all, CLOs form an important but less well-known financier of private equity transactions. The CLO capital structure offers potentially enhanced returns for investors with higher risk tolerance and a long-term investment horizon. Thus, riskier investments in CLO equity are potentially complementary to private equity in terms of return and cash flow profile.

## Bluemetric Insight

CLOs represent an interesting investment category to add to a portfolio, although obtaining exposure to this investment category requires considerable effort. A broad range of fund structures is available on the market, managed by different managers with varying risk profiles, investment styles, economic incentives, and cost structures. Finally, investors must establish appropriate allocation policies to implement their desired level of exposure.`,
}

export default post
